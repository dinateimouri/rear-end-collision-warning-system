%% comments
%Rear End Striking(conflict with a lead vehicle)
%Crash data
...
for i=1:numel(RearEndStrikingEventType)
    if RearEndStrikingEventType(i)==1
        if  Event(i).Sensor.front_radar==1 && Event(i).Sensor.speed==1 && Event(i).Sensor.long_accel==1 && strcmp('Crash' , Event(i).Video.severity)
...
crashdata= MyDataSet_2;
crashdata(:,[6 7 8 9 10])=[];

save crashdata crashdata


%%
crashwarndata = crashdata(crashdata(:,end)==1,:);
crashsafedata = crashdata(crashdata(:,end)==0.5,:);

save crashsafedata crashsafedata
save crashwarndata crashwarndata
